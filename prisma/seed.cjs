...
async function main() {
  ...
  // Helper to generate a mock tracklist (roughly every 7 minutes) if a mix has none
  for (const mix of mixes) {
    const existing = await prisma.track.count({ where: { mixId: mix.id } });
    if (existing > 0) continue;
    const duration = mix.durationSeconds || 7200;
    const interval = 420; // 7 minutes
    ...
    for (let i = 0; i < trackCount; i++) {
      ...
      mockTracks.push({
        index: i + 1,
        timecodeSeconds,
        title: `${mix.artist.split(' ')[0]} Exclusive ID ${i + 1}`,
        artist: i % 3 === 0 ? mix.artist : sampleArtist(i),
        label: sampleLabel(i)
      });
    }
    for (const t of mockTracks) {
      await prisma.track.create({ data: { ...t, mixId: mix.id } });
    }
  }

  // Add sample reviews for Carl Cox only (if none)
  const carl = mixes.find(m => m.number === 4);
  if (carl) {
    const existingReviews = await prisma.review.count({ where: { mixId: carl.id } });
  ...
  console.log('Seed complete (with sample tracks & reviews)');
}
