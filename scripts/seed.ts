...
import { getSequelize, initModels, Mix, Track, Review, RecommendedMix } from '../lib/sequelize';
...
async function main() {
  ...
  for (const data of mixesData) {
    ...
    else mixes.push(await Mix.create(data as any));
  }
  ...
  for (const p of priorities) {
    ...
    else await RecommendedMix.create({ mixId: mix.id, priority: p.priority } as any);
  }
...
function sampleArtist(i: number) {
